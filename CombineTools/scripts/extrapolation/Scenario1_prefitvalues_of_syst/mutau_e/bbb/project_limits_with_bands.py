...
def make_tgraph(my_dict,quantile='center'):
    ret_TGraph=ROOT.TGraph()
    count=0
    lumi_keys=my_dict.keys()
    lumi_keys.sort()
    lumi_keys_rev=copy.copy(lumi_keys)
    lumi_keys_rev.sort(reverse=True)
    if quantile=='center':
        for lumi in lumi_keys:
            ret_TGraph.SetPoint(count,lumi,my_dict[lumi][2])
            count+=1
    elif quantile=='1sigma':
        for lumi in lumi_keys:
            ret_TGraph.SetPoint(count,lumi,my_dict[lumi][3])
            count+=1
        for lumi in lumi_keys_rev:
            ret_TGraph.SetPoint(count,lumi,my_dict[lumi][1])
            count+=1
    elif quantile=='2sigma':
        for lumi in lumi_keys:
            ret_TGraph.SetPoint(count,lumi,my_dict[lumi][4])
            count+=1
        for lumi in lumi_keys_rev:
            ret_TGraph.SetPoint(count,lumi,my_dict[lumi][0])
            count+=1
...
for line in cards_list.readlines():
    ...
    if 'comb' not in current_category[0]:continue
    add_rate_params(current_category[0])
    current_dict=doallLimits(current_category[0],lumilist=lumilist,cat=current_category[1])
    print current_dict,"  ",current_category[1]
    current_graph=make_tgraph(current_dict)
    current_graph_1sigma=make_tgraph(current_dict,'1sigma')
    current_graph_2sigma=make_tgraph(current_dict,'2sigma')
...
frameymax=1.2
framexmax=320
myFrame=canvas.DrawFrame(0.2,0,framexmax,frameymax)
myFrame.GetXaxis().SetTitle("Luminosity [fb^{-1}]")
myFrame.GetYaxis().SetTitle("Expected limit")
#canvas.cd()
...
current_graph.SetTitle('combined '+catMap[channel])
current_graph.SetName('combined '+catMap[channel])
current_graph.SetLineColor(color)
current_graph.SetLineWidth(3)
current_graph.SetMarkerStyle(21)
my_legend.AddEntry(current_graph,'Median',"lp")

#my_mg.Add(graph_list[j][0])
#canvas.Update()

current_graph_1sigma.SetFillColor(ROOT.kGreen)
entry=my_legend.AddEntry(current_graph_1sigma," 1 std deviation","f")
entry.SetFillColor(ROOT.kGreen)
entry.SetFillStyle(1001)
entry.SetLineStyle(1);

current_graph_2sigma.SetFillColor(ROOT.kYellow)
entry2=my_legend.AddEntry(current_graph_2sigma," 2 std deviations","f")
entry2.SetFillColor(ROOT.kYellow)
entry2.SetFillStyle(1001)
entry2.SetLineStyle(1);


current_graph_2sigma.Draw('f')
current_graph_2sigma.GetXaxis().SetTitle("Luminosity [fb^{-1}]");
current_graph_2sigma.GetYaxis().SetTitle("Expected limit");

current_graph_1sigma.Draw('f')

#graph_list[0][0].Draw("C*")
vline=ROOT.TLine(35.862,0,35.862,frameymax)
...
if args.noBBB:
    nobbb=add_nobbb()
    nobbb.Draw('same')
current_graph.Draw("CP")
current_graph.GetXaxis().SetTitle("Luminosity [fb^{-1}]");
current_graph.GetYaxis().SetTitle("Expected limit");

if args.noBBB:
    canvas.SaveAs(channel+"_bands_nobbb.pdf")
else:
    canvas.SaveAs(channel+"_bands.pdf")
"""
