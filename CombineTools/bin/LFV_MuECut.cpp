...
int main(int argc, char* argv[]){
  ...
  string outputFile="lfv_me_13TeV.input";
  string dirInput="";
  bool binbybin=false;
  string name="def";
  ...
  cb.AddObservations({"*"}, {"lfv"}, {"13TeV"}, {"me"}, cats);

  vector<string> bkg_procs = {"GluGluH", "VBFH", "VH", "Zll", "TT", "ST", "EWK", "EWKDiboson", "QCD", "W", "WG"};// "ttH", 
  cb.AddProcesses({"*"}, {"lfv"}, {"13TeV"}, {"me"}, bkg_procs, cats, false);

  vector<string> sig_procs = {"GluGlu", "VBF"};
  cb.AddProcesses(masses, {"lfv"}, {"13TeV"}, {"me"}, sig_procs, cats, true);
  ...
  cb.cp().process(ch::JoinStr({{"Zll"}}))
    .AddSyst(cb, "CMS_DYpTreweight_13TeV", "shape", SystMap<>::init(1.));
  ...
  if(binbybin){
    ...
    bbb.MergeBinErrors(cb.cp().process({"GluGluH", "VBFH", "VH", "Zll", "TT", "ST", "EWK", "EWKDiboson", "W", "WG", "QCD"}));// "ttH",
