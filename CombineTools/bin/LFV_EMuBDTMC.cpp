...
int main(int argc, char* argv[]){
  ...
  string lumi="";
  string inputFile="shapesEMuBDTMC";
  ...
  cb.cp().process(ch::JoinStr({sig_procs, {"GluGluH", "VBFH", "VH", "Zll", "TT", "ST", "EWK", "EWKDiboson", "W", "WG"}}))// "ttH", "embed"
  ...
  cb.cp().process({"WG"})
    .AddSyst(cb, "norm_wg", "lnN", SystMap<>::init(1.05));//1.1
  //cb.cp().process({"WG"})
